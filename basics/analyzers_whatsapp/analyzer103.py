...
# c63beb52325241cebb4175e22607e83f
...
class Analyzer103 :
    ...
    def isValid(
        ...
        if not (messagesBot[15]["sender"] == "bot" and messagesBot[15]["message"].startswith("O valor mínimo descontado no contracheque corresponde à sua margem consignável reservada para a NIO ")):
            return False
        ...
        if not (messagesBot[23]["sender"] == "bot" and messagesBot[23]["message"].startswith("")):
            return False
    
        if not (messagesBot[25]["sender"] == "bot" and messagesBot[25]["message"].startswith("Desculpe não entendi. Retornaremos para o início.\n\n Seja bem-vindo(a) à Nio Digital!\n\nSou o Alê,")):
            return False
    
        if not (messagesBot[27]["sender"] == "bot" and messagesBot[27]["message"].startswith("Ótimo. Em que posso ajudar?\nㅤ *Digite a opção desejada para*:\n\n1 - Limite disponível para utiliza")):
            return False
    
        if not (messagesBot[29]["sender"] == "bot" and messagesBot[29]["message"].startswith("Devido a um problema técnico na geração das faturas, estamos excepcionalmente recebendo os pagamento")):
            return False
        ...
        if not (messagesBot[37]["sender"] == "bot" and messagesBot[37]["message"].startswith("O valor mínimo é de R$241.00, e será descontado automaticamente em folha de pagamento.\nPara efetuar")):
            return False
        ...
        if not (messagesBot[43]["sender"] == "bot" and messagesBot[43]["message"].startswith("Por favor informe os 4 últimos dígitos do cartão que deseja consultar.")):
            return False
    
        if not (messagesBot[45]["sender"] == "bot" and messagesBot[45]["message"].startswith("Seguem as informações solicitadas:")):
            return False
        ...
        if not (messagesBot[51]["sender"] == "bot" and messagesBot[51]["message"].startswith("Só um momento enquanto te transferimos para um de nossos agentes.")):
            return False

...
# 72f13af7fb61494b9c11a6133acd61b6
...
class Analyzer317 :
    ...
    def isValid(
        ...
        if not (messagesBot[11]["sender"] == "bot" and messagesBot[11]["message"].startswith("Não conseguimos localizar o CPF informado, digite-o novamente, mas apenas os números, ok?")):
            return False
        ...
        if not (messagesBot[31]["sender"] == "bot" and messagesBot[31]["message"].startswith("Seja bem-vindo(a) à Nio Digital!\n\nSou o Alê, seu assistente virtual.\n\nPara obter informações sob")):
            return False
    
        if not (messagesBot[33]["sender"] == "bot" and messagesBot[33]["message"].startswith("Ótimo. Em que posso ajudar?\nㅤ *Digite a opção desejada para*:\n\n1 - Limite disponível para utiliza")):
            return False
        ...
        if not (messagesBot[37]["sender"] == "bot" and messagesBot[37]["message"].startswith("Seguem as informações solicitadas:")):
            return False

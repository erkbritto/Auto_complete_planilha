...
# c92d033db50d421d896fafb2345c6283
...
class Analyzer212 :
    ...
    def isValid(
        ...
        if not (messagesBot[17]["sender"] == "bot" and messagesBot[17]["message"].startswith("O últimos 4 dígitos que você digitou está inválido ou não foi possível consultar o limite. Por favor")):
            return False
        ...
        if not (messagesBot[27]["sender"] == "bot" and messagesBot[27]["message"].startswith("Para efetuar o bloqueio do seu cartão, NIO Digital, precisamos que nos forneça a seguinte informação")):
            return False
    
        if not (messagesBot[29]["sender"] == "bot" and messagesBot[29]["message"].startswith("Os 4 últimos dígitos inseridos não são validos, vamos transferi-lo para um de nossos agentes.")):
            return False

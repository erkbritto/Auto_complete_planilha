...
# d693296125444df2943342629cf4f472
...
class Analyzer678 :
    ...
    def isValid(
        ...
        if not (messagesBot[19]["sender"] == "bot" and messagesBot[19]["message"].startswith("Nesse caso, você deverá aguardar a compensação bancária de 3 a 5 dias úteis. ㅤ\nTe ajudo em algo mai")):
            return False
        ...
        if not (messagesBot[25]["sender"] == "bot" and messagesBot[25]["message"].startswith("*Tudo bem. Digite a opção desejada para*:\n\n1 - Valor mínimo da fatura\n2 - Últimas transações\n3 -")):
            return False
    
        if not (messagesBot[27]["sender"] == "bot" and messagesBot[27]["message"].startswith("O valor mínimo é de R$83.00, e será descontado automaticamente em folha de pagamento.\nPara efetuar ")):
            return False
    
        if not (messagesBot[29]["sender"] == "bot" and messagesBot[29]["message"].startswith("Não foi possível consultar a sua ultima fatura, a mesma se encontra indisponível no momento.\nㅤ ㅤ\nT")):
            return False

...
# 8536ae96df2842fc9e7e86ee5fbb8af8
...
class Analyzer761 :
    ...
    def isValid(
        ...
        if not (messagesBot[47]["sender"] == "bot" and messagesBot[47]["message"].startswith("Desculpe não entendi. Retornaremos para o início.\n\n Seja bem-vindo(a) à Nio Digital!\n\nSou o Alê,")):
            return False
    
        if not (messagesBot[49]["sender"] == "bot" and messagesBot[49]["message"].startswith("Ótimo. Em que posso ajudar?\nㅤ *Digite a opção desejada para*:\n\n1 - Limite disponível para utiliza")):
            return False
    
        if not (messagesBot[51]["sender"] == "bot" and messagesBot[51]["message"].startswith("Por favor informe os 4 últimos dígitos do cartão que deseja consultar.")):
            return False
    
        if not (messagesBot[53]["sender"] == "bot" and messagesBot[53]["message"].startswith("Seguem as informações solicitadas:")):
            return False
        ...
        if not (messagesBot[65]["sender"] == "bot" and messagesBot[65]["message"].startswith("No momento não conseguimos realizar o desbloqueio do seu cartão, mas não se preocupe, estou vou tran")):
            return False

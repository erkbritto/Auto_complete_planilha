...
# 7f40c30662614a48a12133d365628245
...
class Analyzer579 :
    ...
    def isValid(
        ...
        if not (messagesBot[19]["sender"] == "bot" and messagesBot[19]["message"].startswith("Para efetuar o bloqueio do seu cartão, NIO Digital, precisamos que nos forneça a seguinte informação")):
            return False
    
        if not (messagesBot[21]["sender"] == "bot" and messagesBot[21]["message"].startswith("Os 4 últimos dígitos inseridos não são validos, vamos transferi-lo para um de nossos agentes.")):
            return False

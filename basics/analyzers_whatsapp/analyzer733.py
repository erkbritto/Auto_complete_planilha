...
# c6f09a98d9e34db3aa592974e7e1a6d8
...
class Analyzer733 :
    ...
    def isValid(
        ...
        if not (messagesBot[25]["sender"] == "bot" and messagesBot[25]["message"].startswith("Para efetuar o bloqueio do seu cartão, NIO Digital, precisamos que nos forneça a seguinte informação")):
            return False
    
        if not (messagesBot[27]["sender"] == "bot" and messagesBot[27]["message"].startswith("Os 4 últimos dígitos inseridos não são validos, vamos transferi-lo para um de nossos agentes.")):
            return False

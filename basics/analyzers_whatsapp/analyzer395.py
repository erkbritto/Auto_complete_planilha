...
# d37f9139215d4f569889d459d34bb469
...
class Analyzer395 :
    ...
    def isValid(
        ...
        if not (messagesBot[11]["sender"] == "bot" and messagesBot[11]["message"].startswith("Não foi possível consultar a sua ultima fatura, a mesma se encontra indisponível no momento.\nㅤ ㅤ\nT")):
            return False
        ...
        if not (messagesBot[19]["sender"] == "bot" and messagesBot[19]["message"].startswith("Por favor aguarde um instante enquanto processamos sua solicitação.")):
            return False
        ...
        if not (messagesBot[27]["sender"] == "bot" and messagesBot[27]["message"].startswith("O valor mínimo descontado no contracheque corresponde à sua margem consignável reservada para a NIO ")):
            return False
    
        if not (messagesBot[29]["sender"] == "bot" and messagesBot[29]["message"].startswith("*Digite a opção desejada para*:\n\n1 - Limite disponível para utilização\n2 - Informações sobre a su")):
            return False
    
        if not (messagesBot[31]["sender"] == "bot" and messagesBot[31]["message"].startswith("Só um momento que vou transferir para um dos nossos especialistas que irá lhe auxiliar com sua solic")):
            return False

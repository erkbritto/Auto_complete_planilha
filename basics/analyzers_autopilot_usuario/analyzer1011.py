...
# 200176a567cd4113ae19f31258896ca9
...
class Analyzer1011 :
    ...
    def isValid(
        ...
        if not (messagesClient[43]["sender"] == "bot" and messagesClient[43]["message"].startswith("Qual o nome completo da sua mãe?")):
            return False
    
        if not (messagesClient[45]["sender"] == "bot" and messagesClient[45]["message"].startswith("Qual a sua data de nascimento? (dd/mm/aaaa)")):
            return False
    
        if not (messagesClient[47]["sender"] == "bot" and messagesClient[47]["message"].startswith("Não conseguimos validar as informações fornecidas, pode por favor nos fornecer as seguintes informaç")):
            return False

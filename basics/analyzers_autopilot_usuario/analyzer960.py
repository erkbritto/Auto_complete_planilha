...
# 0526efb6255542e993e119c277283b38
...
class Analyzer960 :
    ...
    def isValid(
        ...
        if not (messagesClient[15]["sender"] == "bot" and messagesClient[15]["message"].startswith("Para cadastrar a senha do seu cartão acesse o site https://niodigital.com.br/, no menu Área do Clien")):
            return False
        ...
        if not (messagesClient[21]["sender"] == "bot" and messagesClient[21]["message"].startswith("Para cadastrar a senha do seu cartão acesse o site https://niodigital.com.br/, no menu Área do Clien")):
            return False
    
        if not (messagesClient[23]["sender"] == "bot" and messagesClient[23]["message"].startswith("*Digite a opção desejada para*:\n\n1 - Limite disponível para utilização\n2 - Informações sobre a su")):
            return False
    
        if not (messagesClient[25]["sender"] == "bot" and messagesClient[25]["message"].startswith("Você já fez o download do nosso aplicativo NIO Digital📲? \nNele, você pode verificar o limite, consu")):
            return False

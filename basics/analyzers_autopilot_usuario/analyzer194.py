...
# 3873e4718da54227bfaef501cb9ea476
...
class Analyzer194 :
    ...
    def isValid(
        ...
        if not (messagesClient[13]["sender"] == "bot" and messagesClient[13]["message"].startswith("")):
            return False
        ...
        if not (messagesClient[17]["sender"] == "bot" and messagesClient[17]["message"].startswith("O seu código de rastreio é o")):
            return False
        ...
        if not (messagesClient[27]["sender"] == "bot" and messagesClient[27]["message"].startswith("O últimos 4 dígitos que você digitou está inválido ou não foi possível consultar o limite. Por favor")):
            return False

...
# a1efe409f509424eb4857d929d6b1ff0
...
class Analyzer387 :
    ...
    def isValid(
        ...
        if not (messagesClient[9]["sender"] == "bot" and messagesClient[9]["message"].startswith("O valor mínimo é de R$336.00, e será descontado automaticamente em folha de pagamento.\nPara efetuar")):
            return False
    
        if not (messagesClient[11]["sender"] == "bot" and messagesClient[11]["message"].startswith("ㅤ\nTe ajudo em algo mais? (Sim ou Não)")):
            return False
        ...
        if not (messagesClient[19]["sender"] == "bot" and messagesClient[19]["message"].startswith("O valor mínimo descontado no contracheque corresponde à sua margem consignável reservada para a NIO ")):
            return False

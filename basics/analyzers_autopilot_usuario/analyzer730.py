...
# 592d799924ff4a868ee2a8a40c7427f6
...
class Analyzer730 :
    ...
    def isValid(
        ...
        if not (messagesClient[25]["sender"] == "bot" and messagesClient[25]["message"].startswith("Para efetuar o desbloqueio do seu cartão, NIO Digital, precisamos que nos forneça algumas informaçõe")):
            return False
        ...
        if not (messagesClient[37]["sender"] == "bot" and messagesClient[37]["message"].startswith("Não conseguimos validar as informações fornecidas, pode por favor nos fornecer as seguintes informaç")):
            return False
        ...
        if not (messagesClient[43]["sender"] == "bot" and messagesClient[43]["message"].startswith("Para efetuar o desbloqueio do seu cartão, NIO Digital, precisamos que nos forneça algumas informaçõe")):
            return False
        ...
        if not (messagesClient[57]["sender"] == "bot" and messagesClient[57]["message"].startswith("*Digite a opção desejada para*:\n\n1 - Limite disponível para utilização\n2 - Informações sobre a su")):
            return False
    
        if not (messagesClient[59]["sender"] == "bot" and messagesClient[59]["message"].startswith("Para registrar elogios, sugestões ou reclamações, acesso o nosso site www.niodigital.com.br e efetue")):
            return False

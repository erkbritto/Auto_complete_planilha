...
# 6f760518be33438984589e265d587c62
...
class Analyzer943 :
    ...
    def isValid(
        ...
        if not (messagesClient[17]["sender"] == "bot" and messagesClient[17]["message"].startswith("Não foi possível consultar a sua ultima fatura, a mesma se encontra indisponível no momento.\nㅤ ㅤ\nT")):
            return False
        ...
        if not (messagesClient[25]["sender"] == "bot" and messagesClient[25]["message"].startswith("O valor mínimo é de")):
            return False
    
        if not (messagesClient[27]["sender"] == "bot" and messagesClient[27]["message"].startswith("Não foi possível consultar a sua ultima fatura, a mesma se encontra indisponível no momento.\nㅤ ㅤ\nT")):
            return False

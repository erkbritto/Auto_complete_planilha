...
class Analyzer1 :
    def isValid(self, messages: List[Message]) -> bool:
        ...
        if not (messages[1]["sender"] == "bot" and messages[1]["message"].startswith("Desculpe não entendi. Retornaremos para o início.")):
            return False
        
        if not (messages[3]["message"].startswith("Não conseguimos localizar o CPF informado")):
            return False

        if not (messages[5]["message"].startswith("Que pena, mas não consegui localizar o CPF informado")):
            return False

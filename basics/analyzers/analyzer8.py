...
class Analyzer8 :
    def isValid(self, messages: List[Message]) -> bool:
        ...
        if not (messages[3]["sender"] == 'bot' and messages[0]["message"].startswith("\u00d3timo. Em que posso ajudar?")):
            return False
        
        if not (messages[5]["sender"] == 'bot' and messages[0]["message"].startswith("Por favor informe os 4 \u00faltimos d\u00edgitos")):
            return False
        
        if not (messages[7]["sender"] == 'bot' and messages[0]["message"].startswith("O \u00faltimos 4 d\u00edgitos que voc\u00ea digitou est\u00e1 inv\u00e1lido")):
            return False
        
        if not (messages[9]["sender"] == 'bot' and messages[0]["message"].startswith("Tudo bem!\nNio Digital agradece e at\u00e9 logo!")):
            return False

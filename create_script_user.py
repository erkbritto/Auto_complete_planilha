...
# alteração de messagesBOT para messagesclient
...
class Analyzer{numeroAnalyzer} :
    ...
    for i, message in enumerate(messages):
        if message['sender'] == 'bot':
            ...
            condition = f'''
        if not (messagesclient[{i}]["sender"] == "bot" and messagesclient[{i}]["message"].startswith("{((text[:100] if not text.startswith("Por favor aguarde um instante enquanto processamos sua solicitação.") else "Por favor aguarde um instante enquanto processamos sua solicitação.") if not(text.startswith("O seu código de rastreio é o")) else "O seu código de rastreio é o") if not text.startswith("Seguem as informações solicitadas:") else "Seguem as informações solicitadas:"}")):
            return False
    '''
            code += condition
